...
use braid_event_stream::{BraidEventStream, BraidEvent, KalmanEstimates};
use tokio;
use url::Url;
...
impl LensController {
    ...
    fn is_in_zone(&self, estimates: &KalmanEstimates) -> bool {
        estimates.x >= self.tracking_zone.x_min && estimates.x <= self.tracking_zone.x_max &&
        estimates.y >= self.tracking_zone.y_min && estimates.y <= self.tracking_zone.y_max &&
        estimates.z >= self.tracking_zone.z_min && estimates.z <= self.tracking_zone.z_max
    }
                    ...
                    match braid_event {
                        BraidEvent::Birth(estimates) | BraidEvent::Update(estimates) => {
                            if self.is_in_zone(&estimates) {
                                if self.currently_tracked_obj.is_none() || self.currently_tracked_obj == Some(estimates.obj_id) {
                                    self.currently_tracked_obj = Some(estimates.obj_id);
                                    self.update_lens_position(estimates.z, received_time);
                                    info!("Tracking object {}: z = {}", estimates.obj_id, estimates.z);
                                }
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let update_interval_ms = args.get(3)
        ...
        .unwrap_or(20); // Default to 20ms if not provided
